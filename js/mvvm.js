$(function()
{
    ...
    var vm = avalon.define(
    {
        ...
        disabled_other_columns: function(e)
        {
            ...
            if (this.selected.column.length === 0)
            { //检测到本列数据为空时，还原其他列,并清空已有数据
        ...
        submit_all_keywords: function()
        {
            ...
            else
            {
                ...
                getAjax("../data/search_result/result22.json", null, function(data)
                { //文章查询结果
                    if (data.status === 0)
                    {
        ...
        { //更新页面content的内容
            ...
            {
                getAjax("../data/content1/" + data_id + ".json", null, function(data)
                { //文章内容和slider第一页
        ...
        { //更新slider content指代消解后的的内容
            ...
            if (data1.newsId !== "home-tab")
            {
                getAjax("../data/after1/" + data_id + ".json", null, function(data)
                { //指代消解后
                    // postAjax(url,data,function(data){
                    if (data.NewsId == data_id)
                    {
        ...
        //底部tab分页滑动控制
        change_keywords_slider_up: function(e)
        { //向上滑动
        ...
        change_keywords_slider_down: function(e)
        { //向下滑动
            var thisSlide = $(e.target).parent();
            var index = $(".keywords_slider").index(thisSlide);
            var next = index + 1;
        ...
        {
            getAjax("../data/chartsData1/" + id + ".json", null, function(data)
            { //设置图表1
                if (data.Status === 0)
                {
        ...
        {
            //设置图表二
            getAjax("../data/chartsData2/" + id + ".json", null, function(data)
            { //设置图表2
                if (data.Status === 0)
                {
